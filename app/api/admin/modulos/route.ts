  ...
  try {
    ...
    if (tipo === 'modulo') {
      ...
      const { data: existing } = await supabaseAdmin
        .from('subcategorias')
        .select('id, nombre, prefijo_codigo')
        .eq('modulo_id', data.modulo_id)
        .eq('prefijo_codigo', data.prefijo_codigo.toUpperCase())
        .single();

      if (existing) {
        return NextResponse.json(
          { error: `El prefijo '${data.prefijo_codigo.toUpperCase()}' ya está en uso por '${existing.nombre}' en este módulo` },
          { status: 409 }
        );
      }
      ...
      const { data: subcategoria, error } = await supabaseAdmin
        .from('subcategorias')
        .insert({ nombre: data.nombre, modulo_id: data.modulo_id, prefijo_codigo: data.prefijo_codigo.toUpperCase() })
        .select()
        .single();
...
export async function PUT(request: Request) {
  try {
    ...
    if (tipo === 'modulo') {
      ...
      const { error } = await supabaseAdmin
        .from('subcategorias')
        .update({ nombre: data.nombre, prefijo_codigo: data.prefijo_codigo.toUpperCase() })
        .eq('id', id);
